...
flowplot1<-ggplot(data=FT1, aes(interaction(Env.x,Sex.x),yvar, lty=Sex.x)) +
  geom_errorbar(aes(ymin=FT1$yvar-FT1$SE, ymax=FT1$yvar+FT1$SE, lty=Sex.x), position=position_dodge(width=0.75), alpha=.8,width=.6) +
  #geom_smooth(data=test2, lty="dashed",aes(alpha=.8)) +
  theme_bw() +
  xlab("Environment") +
  ylab("Flowering Time\n(days)") +
